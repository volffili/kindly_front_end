;(function () {
  ...
  function initHeader() {
    ...
    for (var i in points) {
      var c = new Circle(points[i], 1 + Math.random(), "rgba(255,255,255,0.3)")
      points[i].circle = c
    }
  }
  ...
  function shiftPoint(p) {
    TweenLite.to(p, 1 + 1 * Math.random(), {
      x: p.originX - 25 + Math.random() * 50,
      y: p.originY - 25 + Math.random() * 50,
      ease: Circ.easeInOut,
      onComplete: function () {
        shiftPoint(p)
      },
  ...
  function drawLines(p) {
    ...
    for (var i in p.closest) {
      ...
      ctx.strokeStyle = "rgba(71,78,99," + p.active + ")"
      ctx.stroke()
    }
  }
  ...
  function Circle(pos, rad, color) {
    ...
    this.draw = function () {
      ...
      ctx.fillStyle = "rgba(71,78,99," + _this.active + ")"
      ctx.fill()
    }
  }
